  ...
  return (
    <main className="min-h-screen bg-[#F4F7FB] px-6 py-16 text-night lg:px-16">
      <section className="mx-auto max-w-6xl">
        ...
        <h1 className="mt-6 text-5xl font-black">Estimez les economies annuelles avec EnterpriseERP.</h1>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[.9fr_1.1fr]">
          <section className="rounded-3xl bg-white p-7 shadow ring-1 ring-slate-200">
            <h2 className="text-2xl font-black">Vos donnees</h2>
            <div className="mt-6 grid gap-5">
              <Field label="Nombre d'employes" value={employees} onChange={setEmployees} />
              <Field label="Heures administratives par employe / semaine" value={adminHours} onChange={setAdminHours} suffix="h" />
              <Field label="Factures mensuelles" value={invoices} onChange={setInvoices} />
              <Field label="Factures en retard" value={lateRate} onChange={setLateRate} suffix="%" />
              <Field label="Cout horaire moyen" value={hourlyCost} onChange={setHourlyCost} suffix="EUR" />
            </div>
          </section>

          <section className="rounded-3xl bg-[#101b2d] p-7 text-white shadow-xl">
            <p className="text-sm font-black uppercase tracking-[0.18em] text-[#7df5e5]">Projection</p>
            <h2 className="mt-3 text-3xl font-black">Valeur business estimee</h2>
            <div className="mt-7 grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl bg-white/10 p-5">
                <p className="text-sm text-white/65">Gain temps administratif</p>
                <p className="mt-2 text-3xl font-black">{Math.round(result.automatedSavings).toLocaleString("fr-FR")} EUR</p>
              </div>
              <div className="rounded-2xl bg-white/10 p-5">
                <p className="text-sm text-white/65">Encaissements recuperables</p>
                <p className="mt-2 text-3xl font-black">{Math.round(result.expectedRecovery).toLocaleString("fr-FR")} EUR</p>
              </div>
              <div className="rounded-2xl bg-[#00C2A9] p-5 text-night sm:col-span-2">
                <p className="text-sm font-black uppercase">Economies annuelles estimees</p>
                <p className="mt-2 text-5xl font-black">{Math.round(result.annualSavings).toLocaleString("fr-FR")} EUR</p>
              </div>
              <div className="rounded-2xl bg-white/10 p-5 sm:col-span-2">
                <p className="text-sm text-white/65">Delai de retour estime</p>
                <p className="mt-2 text-3xl font-black">{Math.max(1, Math.round(result.paybackMonths)).toLocaleString("fr-FR")} mois</p>
              </div>
            </div>
          </section>
        </div>
